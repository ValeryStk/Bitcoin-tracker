...
void saveLastBitcoinPrice(const double btcPrice) {
    QJsonObject obj;
    obj["lastFetchedBtcPrice"] = btcPrice;
    QJsonDocument doc(obj);
    QFile file(QApplication::applicationDirPath() + "/" + "last_btc_price");
    if (file.open(QIODevice::WriteOnly)) {
        file.write(doc.toJson(QJsonDocument::Indented));
        file.close();
    }
...
double loadLastBitcoinPrice() {
    ...
    return data["lastFetchedBtcPrice"].toDouble();
};
...
void BitcoinBiperMainWindow::updateBtcPrice() {
    ...
    ttm->recalculateSummary();
    updateProfit();
}

void BitcoinBiperMainWindow::updateProfit() {
